...
class ChangePointModel_Variational:
    """
    ...
    def plot_results(self, results_df, figsize=(14, 12)):
        ...
        if 'TruePosition' in results_df.columns:
            ax.plot(results_df['Trial'], results_df['TruePosition'], 
                   'k--', label='True Position', linewidth=2, alpha=0.7)
        ax.plot(results_df['Trial'], results_df['Belief'], 
               'b-', label='Model Belief (μ¹)', linewidth=1.5)
        ax.fill_between(results_df['Trial'], 
                        results_df['Belief'] - results_df['Uncertainty'],
                        results_df['Belief'] + results_df['Uncertainty'],
                        alpha=0.2, color='blue', label='Uncertainty (±σ¹)')
        ...
        ax = axes[1]
        ax.plot(results_df['Trial'], results_df['LearningRate'], 
               'g-', linewidth=1.5, label='Learning Rate (α¹)')
        ...
        ax = axes[2]
        ax.plot(results_df['Trial'], results_df['CPP'], 
               'orange', linewidth=1.5, label='Change-Point Probability (Ω)')
        ...
        if self.add_second_level and 'Mu2' in results_df.columns:
            ax = axes[3]
            ax.plot(results_df['Trial'], results_df['Mu2'], 
                   'purple', linewidth=1.5, label='Volatility (μ²)')
...
if __name__ == "__main__":
    ...
    cpm_model_change = ChangePointModel_Variational(
    ...
    cpm_model_walk = ChangePointModel_Variational(
