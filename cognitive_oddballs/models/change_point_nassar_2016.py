...
class ChangePointNassarModel:
    ...
    def plot_results(
        self, results_df, env_df=None, noise_switch_trial=None, zoom_start=180, zoom_end=220
    ):
        """
        ...
        if "TruePosition" in results_df.columns:
            plt.plot(
                ...
                "k--",
                label="True Helicopter",
                linewidth=2,
            )

        # Plot model belief
        plt.plot(results_df["Trial"], results_df["Belief"], "b-", label="Model Belief", linewidth=2)

        # Mark oddball trials if environment data provided
        if env_df is not None and "is_oddball" in env_df.columns:
            oddball_trials = (
                env_df[env_df["is_oddball"]]["trial"] + 1
            )  # +1 because Trial starts at 1
        ...
        if noise_switch_trial is not None:
            ...
            plt.axvspan(
                noise_switch_trial, len(results_df), color="red", alpha=0.1, label="High Noise"
            )
        ...
        plt.plot(
            results_df["Trial"],
            results_df["RelUncertainty"],
            color="purple",
            label="Relative Uncertainty (τ)",
            linewidth=1.5,
        )
        ...
        pred_error_norm = (
            pred_error_zoom / np.max(pred_error_zoom)
            if np.max(pred_error_zoom) > 0
            else pred_error_zoom
        )

        rel_unc_zoom = results_df["RelUncertainty"][zoom_mask]
        rel_unc_norm = (
            rel_unc_zoom / np.max(rel_unc_zoom) if np.max(rel_unc_zoom) > 0 else rel_unc_zoom
        )
        ...
        plt.plot(
            ...
            rel_unc_norm,
            color="purple",
            label="Relative Uncertainty (τ, normalized)",
            linewidth=2,
        )
        ...
        # Add noise shading if switch point provided
        if noise_switch_trial is not None and zoom_start < noise_switch_trial < zoom_end:
